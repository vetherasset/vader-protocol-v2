...
contract("VaderPool V2", (accounts) => {
    ...
    describe("burn", () => {
        it("should burn liquidity and check the state", async () => {
            const { poolV2, dai, mockUsdv } = await deployMock();

            // TODO
            // await poolV2.burn(1, accounts.account0);
            // console.log((await dai.balanceOf(accounts.account3)).toString());
            // console.log(
            //     (await mockUsdv.balanceOf(accounts.account3)).toString()
            // );
        });
    });
