...
module.exports = (artifacts) => {
  ...
  const assertBn = (a, b, deviance, debug) => {
    if (deviance) {
      ...
      return (
        assert.ok(b.add(deviance).gte(a)) && assert.ok(b.sub(deviance).lte(a))
      )
  ...
  const assertEvents = ({ logs }, events) => {
    ...
    names.forEach((name) => {
      ...
      for (let log of specificLogs) {
        ...
        for (let argSet of events[name]) {
          ...
          matched =
            ...
            args.every((arg) => {
              ...
              if (typeof value === 'string' && value !== argSet[arg]) {
        ...
        if (!matched)
          assert.fail(`Event ${name} Did Not Match Any Argument Sets`)
      }
    })
  }
  ...
  const deployMock = async (accounts, configs) => {
    ...
    cached.mockUSDV = await MockToken.new('USD Vader', 'USDV', 18)
