...
module.exports = ({ artifacts, parseUnits, big }) => {
    // Mocks
    ...
    const prepareTargetsAndData = async ({
        ...
        cached.targets = await basicArray.reduce(
            async (acc) => [
                ...(await acc),
        ...
        const targetAddresses = basicArray.map(
            (_, idx) => cached.targets[idx].address
        );
    ...
    const advanceBlockToVotingPeriodEnd = async ({ governorAlpha }) => {
        const votingPeriod = await governorAlpha.votingPeriod();
        await time.advanceBlockTo(
            (await web3.eth.getBlock("latest")).number + votingPeriod.toNumber()
        );
    };

    const getTxHash = (args) =>
